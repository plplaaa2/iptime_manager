...
* **스마트 캐싱 & 백그라운드 스캔**: 동기적인 API 통신으로 인해 홈어시스턴트 UI가 얼어붙는 현상을 방지하고자, 1시간 스마트 캐싱과 백그라운드 태스크 엔진을 도입해 쾌적하게 무선 가용 채널 목록을 조회하고 갱신합니다.
* **채널 속성 실시간 노출**: Wi-Fi 온오프 스위치의 상태 속성(`channel` 필드)에 현재 무선 칩셋의 물리 가동 채널 정보를 실시간으로 결합해 제공합니다.

### 4. 개별 Wi-Fi SSID(BSS) 토글 스위치
...
### 10. 공유기 물리 유선 포트 링크 단절/연결 실시간 커스텀 이벤트 (1.0.3 신설)
* **포트 상태 천이 감지**: LAN 1~4 및 WAN 물리 포트의 이더넷 케이블 탈착(Link Up / Down)을 실시간으로 감지합니다.
* **알림 폭탄 방지**: 단순 노트북 재부팅 등으로 수많은 알림 팝업창이 뜨는 오버헤드를 막기 위해, 알림 대신 지능형 백그라운드 커스텀 이벤트(`iptime_manager_port_connected`, `iptime_manager_port_disconnected`)를 방출합니다.
* **풍부한 자동화 페이로드**: 포트 종류(lan/wan), 포트 번호, 포트 표기명, 감지된 링크 연결 물리 속도(Gbps/Mbps) 등을 이벤트 데이터에 동봉하여 지능형 스마트홈 기기 전원 제어나 경보 자동화 연동을 완벽히 지원합니다.
...
| **`select`** | 나이트 LED 모드 설정, 자동 재부팅 요일 설정, GeoIP 정책 설정, **[신설] Wi-Fi 대역별 채널 설정 선택 (`select.iptime_wifi_channel_...`)** |
| **`button`** | 공유기 즉시 안전 재부팅 (`button.reboot`) |
| **`Event Bus`** | **[신설] 물리 포트 연결/해제 이벤트 (`iptime_manager_port_connected` / `_disconnected`), 보안 경보 이벤트 (`iptime_manager_security_alert`), WAN 단선/IP 변경 이벤트 (`iptime_manager_wan_alert`)** |
