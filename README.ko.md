...
재실 추적기는 단일 공유기 또는 EasyMesh 컨트롤러가 등록돼 있을 때 만들 수 있습니다. 선택한 기기는 Agent를 포함해 등록된 모든 공유기에서 감지하며, 기기별 추적기는 해당 기기가 감지되면 `home`, 감지되지 않으면 `not_home` 상태가 됩니다. 추적기들은 `Home Presence` 기기 아래에 표시됩니다.
