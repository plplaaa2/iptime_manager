...
### ⏱️ 초정밀 실시간-웹 이원화 수집 및 스캔 주기(scan_interval) 커스텀화
* **자유로운 초 단위 주기 선택**: 최초 통합구성요소 추가 폼 및 연동 후 구성 변경(Options Flow) 화면에서 사용자가 원하는 재실 기기 갱신 주기(초)를 직접 커스텀 입력할 수 있습니다.
* **이원화 격리 수집 최적화 (공유기 부하 원천 차단)**: 재실 기기 반응 속도를 끌어올리기 위해 스캔 주기를 1~3초로 극단적으로 짧게 가져가더라도, 무거운 시스템/포트 링크/무선 상세 설정 데이터 수집은 **최소 5초 주기로 스로틀링(Throttling) 작동**하도록 격리 독립시켰습니다.
* **제어 0초 실시간 반응 (바이패스 장치)**: 5초 대기 상태 중이라도 사용자가 대시보드 상에서 와이파이 스위치나 LED 모드 등 제어 조작을 내리면, 대기 시간 없이 **즉각적으로 강제 동기화 갱신이 진행되어 최고 수준의 조작 피드백 속도**를 선사합니다.
* **기기 보호막**: CPU 및 통신 칩셋 자원이 부족한 보급형/구형 공유기 기종에서도 불필요한 트래픽 쿼리 과부하로 인한 기기 다운 현상 걱정 없이 안심하고 사용할 수 있는 스마트 세이프티 아키텍처입니다.
